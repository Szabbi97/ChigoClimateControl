...
void handleRoot() {
      ...
      <script>
        ...
        function setTemperature() {
          ...
          fetch('/set?temp=' + temp)
            .then(response => response.text())
            .then(text => document.getElementById('status').innerText = text);
          return false; // ne küldje el a formot
        }
      </script>
    </head>
    <body>
      <h2>Chigo klíma vezérlés</h2>
      <button onclick="sendCommand('/on')">Bekapcsolás</button>
      <button onclick="sendCommand('/off')">Kikapcsolás</button>
      <form onsubmit="return setTemperature();">
        <label>Hőmérséklet:</label>
        <select id="temp" name="temp">
  )rawliteral";

  for (int i = 18; i <= 30; i++) {
    html += "<option value='" + String(i) + "'>" + String(i) + " °C</option>";
  }
  ...
  html += R"rawliteral(
      ...
      <p id="status">Állapot: nincs művelet</p>
    </body>
    </html>
  )rawliteral";
...
void handleSet() {
  if (server.hasArg("temp")) {
    ...
    switch (temp) {
      case 18: irsend.sendRaw(rawData_18, 99, 38); break;
      case 19: irsend.sendRaw(rawData_19, 99, 38); break;
      case 20: irsend.sendRaw(rawData_20, 99, 38); break;
      case 21: irsend.sendRaw(rawData_21, 99, 38); break;
      case 22: irsend.sendRaw(rawData_22, 99, 38); break;
      case 23: irsend.sendRaw(rawData_23, 99, 38); break;
      case 24: irsend.sendRaw(rawData_24, 99, 38); break;
      case 25: irsend.sendRaw(rawData_25, 99, 38); break;
      case 26: irsend.sendRaw(rawData_26, 99, 38); break;
      case 27: irsend.sendRaw(rawData_27, 99, 38); break;
      case 28: irsend.sendRaw(rawData_28, 99, 38); break;
      case 29: irsend.sendRaw(rawData_29, 99, 38); break;
      case 30: irsend.sendRaw(rawData_30, 99, 38); break;
      default:
